...
class SupabaseGeographyRepository implements GeographyRepository {
  ...
  Future<Result<Zone>> saveZone(Zone zone) {
    ...
    if (zone.id.isEmpty) {
      return Result.guard(() async {
        final saved = await _db.from('zones').insert(row).select().single();
        return _zone(saved);
      });
    }
    return Result.guardWrite(
      () => _db.from('zones').update(row).eq('id', zone.id).select(),
      _zone,
    );
  }
  ...
  Future<Result<Landmark>> saveLandmark(Landmark landmark) {
    ...
    if (landmark.id.isEmpty) {
      return Result.guard(() async {
        final saved = await _db.from('landmarks').insert(row).select().single();
        return _landmark(saved);
      });
    }
    return Result.guardWrite(
      () => _db.from('landmarks').update(row).eq('id', landmark.id).select(),
      _landmark,
    );
  }
  ...
  Future<Result<void>> deleteLandmark(String landmarkId) {
    return Result.guardWrite(
      () => _db.from('landmarks').delete().eq('id', landmarkId).select('id'),
      (_) {},
    );
  }
...
class FakeGeographyRepository implements GeographyRepository {
  ...
  Future<Result<Zone>> saveZone(Zone zone) async {
    if (failure != null) return Result.err(failure!);
    if (zone.id.isNotEmpty && !_zones.any((existing) => existing.id == zone.id)) {
  ...
  Future<Result<Landmark>> saveLandmark(Landmark landmark) async {
    ...
    if (landmark.id.isNotEmpty &&
        !_landmarks.any((existing) => existing.id == landmark.id)) {
      return const Result.err(NotFoundFailure());
    }
    final saved =
  ...
  Future<Result<void>> deleteLandmark(String landmarkId) async {
    if (failure != null) return Result.err(failure!);
    if (!_landmarks.any((landmark) => landmark.id == landmarkId)) {
      return const Result.err(NotFoundFailure());
    }
