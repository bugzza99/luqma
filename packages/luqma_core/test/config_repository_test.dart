...
void main() {
  group('FakeConfigRepository', () {
    test('readAll returns the seeded rows already typed', () async {
      final repo = FakeConfigRepository(seed: {'otp_enabled': true, 'marketing_push_per_week': 3});
    ...
    test('setValues upserts and records exactly what the screen wrote', () async {
      final repo = FakeConfigRepository();

      await repo.setValues({'otp_enabled': true, 'support_whatsapp': '010'});
