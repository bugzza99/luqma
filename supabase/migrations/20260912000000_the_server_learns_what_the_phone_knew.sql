...
--
-- Each one has the same shape: `CLAUDE.md` records the rule, the Dart applies it, and
-- `place_order_priced` — the only authority that counts, because it is what a courier
-- collects against — had never been told. The suite stayed green throughout, because the
-- fakes are more permissive than Postgres and no widget test leaves the phone.
--
-- The body below is the current function with four changes and nothing else; every other
-- line is byte-for-byte what was there.
