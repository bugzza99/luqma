-- The extras are priced by the menu, like everything else on the order.
--
-- `place_order` exists because the phone's numbers are not trusted: it reads the dish's
-- price out of `menu_items` rather than believing the draft. That held for the base price
-- and **not** for the extras — `optionsTotal` arrived as a number and was added verbatim,
-- guarded only by `>= 0`.
--
-- So a crafted request could order every extra on the menu and claim they were free. The
-- merchant hands over the food, collects the base price in cash, and nothing anywhere
-- records that the difference existed. The anon key is public and sits inside every APK,
-- so nothing about that is difficult.
--
-- The client always had what was needed: the cart holds the chosen `MenuOption` objects,
-- ids and all, and flattened them to a single number on the way out. It sends the ids
-- now, and `optionsTotal` is ignored — a number nobody can verify is a number nobody
-- should read.
--
-- An id the dish does not have is refused rather than dropped: silently ignoring it would
-- charge for less than was ordered, which is the same hole with better manners.

create or replace function public.price_line_options(
  p_options jsonb,
  p_chosen  jsonb
)
returns integer
...
declare
  ...
  v_seen  text[] := '{}';
begin
  if p_chosen is null or jsonb_typeof(p_chosen) <> 'array' then
    return 0;
  end if;

  for v_id in select jsonb_array_elements_text(p_chosen) loop
    -- The same extra twice is one extra. A repeated id is a client bug, not an order for
    -- two sauces, and charging twice for it would be the mirror of the hole above.
    if v_id = any(v_seen) then
      continue;
    end if;
    v_seen := v_seen || v_id;

    select (o ->> 'price')::int into v_price
      from jsonb_array_elements(coalesce(p_options, '[]'::jsonb)) o
     where o ->> 'id' = v_id;

    if v_price is null then
      raise exception 'no such option on this item' using errcode = 'P0001';
    end if;
...
-- And the function that used to trust the draft.
--
-- Replaced whole because Postgres cannot patch a body; only three statements inside it
-- differ, marked where they are. Everything else is `20260826020000_launch_fixes.sql` as
-- it stood.
...
begin
  ...
  for v_line in select * from jsonb_array_elements(v_items) loop
    ...
    if v_is_preorder then
      ...
      select name, price, options into strict v_menu
